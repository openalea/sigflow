...
def add_axis_coordinates(g,scale_):
    for vid in g.vertices(scale_):
        ...
        axis_node.XX = old_div((first_node.XX + last_node.XX),2)
        axis_node.YY = old_div((first_node.YY + last_node.YY),2)
        axis_node.ZZ = old_div((first_node.ZZ + last_node.ZZ),2)
        axis_node.length = sqrt((first_node.XX - last_node.XX)**2 + (first_node.YY + last_node.YY)**2 + (first_node.ZZ + last_node.ZZ)**2)
        axis_node.semilength = old_div(axis_node.length,2)
...
def compute_activating_inhibiting_signal_terminal_shoots_only(
    g, selected_scale, alpha_inib, alpha_activ, dists):
    """
    """    
    ax_terms = [aid for aid in g.vertices(selected_scale) if g.node(aid).nb_children() == 0]
    print(ax_terms)
    ...
    for vid in ax_terms:
        ...
        for jid in ax_terms:
            ...
            match_node.current_inhibiting = node.ini_inibiting * (old_div(1, (1+dists[vid,jid])))**alpha_inib
            match_node.current_activating = node.ini_activating * 10000 * (old_div(1, (1+dists[vid,jid])))**alpha_activ 
            #print('-------------------------------')
        ...
        for jid in ax_terms:
            ...
            if (node.ini_inibiting != 0) :
                match_node.inhibiting +=  old_div(match_node.current_inhibiting  * node.ini_inibiting, node.tot_inhibiting) * factor
                node.self_inhibiting = old_div(node.self_inhibiting * node.ini_inibiting, node.tot_inhibiting)
            if (node.ini_activating !=0) :
                match_node.activating += old_div(match_node.current_activating * node.ini_activating, node.tot_activating)    * factor
                node.self_activating = old_div(node.self_activating * node.ini_activating, node.tot_activating)
...
def meristem_fates(
    g, selected_scale, 
    value_at_05_fruits=0.5, 
    shape_factor_fruits=0.5, 
    value_at_05_leaves = 2, 
    shape_factor_leaves = 2):
    """
...
def probability_value(x,b=0.5,c=0.5):
    #print([x,b,c])
    return (old_div(1, (1 + exp(old_div(-(x-b),c)))))
